...
Each signature may define:

- `semantic_kind`: `primitive`, `abstract`, `either`, `external_event`, or `out_of_planning_scope`;
- `parameters`: explicit typed roles, including the actor and complements;
...
- `world_effects`: changes to ground-truth physical or operational state;
- `knowledge_effects`: changes to force-relative knowledge or belief state;
- `resource_effects`: changes to consumable or available resources;
- `completion`: observable success conditions;
- `failure_outcomes`: named failure outcomes and the state concepts they establish;
- `execution_responsibility`: the layer expected to execute or supervise the task.
...
semantics:
  ...
  failure_outcomes: []
  execution_responsibility: platform_autonomy
```
  ...
  semantics:
    ...
    world_effects:
      ...
      - state: located_at
        bindings: {entity: actor, location: destination}
      remove:
      - state: located_at_previous_location
        bindings: {entity: actor}
    knowledge_effects:
      add: []
      remove: []
    resource_effects: []
...
- signature_id: TC-001-S02
  ...
  semantics:
    ...
    world_effects:
      ...
      - state: located_at
        bindings: {entity: actor, location: route_destination}
      remove: []
    knowledge_effects:
      add: []
      remove: []
...
- signature_id: TC-006-S01
  ...
  semantics:
    ...
    completion:
      success:
      - state: follow_terminated_by_superior_task
        bindings: {follower: actor, target: followed_unit}
    failure_outcomes:
    - outcome: target_lost
      establishes:
  ...
  semantics:
    ...
    - outcome: escort_capability_lost
      ...
      - state: escort_degraded
        bindings: {escort: escort_actor, protected: protected_unit}
    execution_responsibility: level_1_planner
traceability:
  used_by_missions:
  - MC-023
  - MC-024
