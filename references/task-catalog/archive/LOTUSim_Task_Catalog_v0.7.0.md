...
Every binding variable resolves to a typed parameter declared by the same signature. Parameter values are either supplied as `task_input` or captured explicitly from `state_at_start`. Removal effects identify the exact bound tuple to remove; pseudo-states representing an unknown previous value are forbidden.
...
semantic_families:
  - id: SF-01
    ...
    common_semantics:
      ...
      transitions: [unknown_to_detected, detected_to_localized, localized_to_classified, classified_to_identified, track_established_or_updated]
