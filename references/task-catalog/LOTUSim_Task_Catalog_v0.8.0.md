...
State references are initially expressed as controlled candidate names. They shall subsequently resolve to stable State Model identifiers. The Task Catalog references State Model entries but does not redefine them.
...
semantic_families:
  - id: SF-01
    ...
    manipulated_concepts: [observation, entity, emission, area, location, classification, identity, track]
    common_semantics:
      reads: [sensor_availability, observation_context]
      invariants: [world_knowledge_separation, force_relative_knowledge]
      transitions: [area_observed, entity_or_emission_detected, location_estimate_produced, classification_assessed, identity_assessed, track_established_or_updated]
...
The types `InformationHolder`, `ObservationRecord`, `DetectionEvidence`, `EmissionDetectionEvidence`, `LocationEstimate`, `ClassificationAssessment`, `IdentityAssessment`, `Track` and `Emission` are controlled candidates for the first State Model. They are not silently introduced as Naval Ontology classes.
...
| `detected` | Detect Object | Localize, Classify, Identify, Track |
| `emission_detected` | Detect Emission | Future Characterize Emission increment |
...
- signature_id: TC-006-S01
  ...
  semantics:
    ...
    - state: target_location_known
      bindings: {observer: actor, target: followed_unit}
    invariants:
    - state: target_location_known
      bindings: {observer: actor, target: followed_unit}
    - state: capability_available
...
- signature_id: TC-011-S01
  ...
  semantics:
    ...
    - outcome: observation_quality_insufficient
      establishes:
      - state: observation_attempt_inconclusive
  ...
  semantics:
    ...
    operational_effects:
      ...
      knowledge:
        ...
        - state: detection_evidence_available
    ...
    - outcome: detection_threshold_not_met
      establishes:
      - state: detection_attempt_inconclusive
...
- signature_id: TC-012-S02
  ...
  semantics:
    ...
    - role: emission
      ...
      source: task_input
    - role: evidence
      type: EmissionDetectionEvidence
      source:
        kind: execution_output
    ...
    operational_effects:
      ...
      knowledge:
        add:
        - state: emission_detected
          bindings: {holder: knowledge_holder, emission: emission}
        - state: emission_detection_evidence_available
    ...
    - outcome: detection_threshold_not_met
      establishes:
      - state: emission_detection_attempt_inconclusive
...
- signature_id: TC-013-S01
  ...
  semantics:
    ...
    - outcome: localization_quality_insufficient
      establishes:
      - state: localization_attempt_inconclusive
        bindings: {holder: knowledge_holder, actor: actor, target: target}
  ...
  semantics:
    ...
    operational_effects:
      ...
      knowledge:
        ...
        - state: classification_available
          bindings: {holder: knowledge_holder, target: target}
        remove: []
      resources:
        add: []
    ...
    - outcome: classification_quality_insufficient
      establishes:
      - state: classification_attempt_inconclusive
        bindings: {holder: knowledge_holder, actor: actor, target: target}
  ...
  semantics:
    ...
    applicability:
      ...
      - any_of:
        - state: classification_available
          bindings: {holder: knowledge_holder, target: target}
        - state: direct_identity_evidence_available
          bindings: {holder: knowledge_holder, target: target}
      execution:
      - state: capability_available
        bindings: {entity: actor, capability: IdentificationCapability}
    ...
    - outcome: identity_evidence_insufficient
      establishes:
      - state: identification_attempt_inconclusive
        bindings: {holder: knowledge_holder, actor: actor, target: target}
    - outcome: processing_capability_lost
      establishes:
      - state: capability_unavailable
...
- signature_id: TC-023-S01
  ...
  semantics:
    ...
    - role: protected_unit
      ...
      source: task_input
    reads:
    - state: assigned_to_escort
      bindings: {escort: escort_actor, protected: protected_unit}
    - state: protection_capability_available
      bindings: {entity: escort_actor}
    - state: protected_unit_location_known
      bindings: {observer: escort_actor, protected: protected_unit}
    invariants:
    - state: protection_capability_available
      bindings: {entity: escort_actor}
    applicability:
    ...
    operational_effects:
      ...
      resources:
        ...
        remove: []
    desired_outcomes:
    - state: protected_unit_at_destination
      bindings: {protected: protected_unit}
    - state: protected_unit_preserved
      bindings: {protected: protected_unit}
    completion_conditions:
    - state: protected_unit_at_destination
      bindings: {protected: protected_unit}
