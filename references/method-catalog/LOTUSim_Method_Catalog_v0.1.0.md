...
Version 0.1.0 is deliberately limited to `TC-023-S01 Escort Unit`. It tests the method metamodel before decomposition is generalized to other abstract signatures.
...
projection:
  readiness: partial
  blockers:
  - HDDL profile must define how continuous subtasks spanning a bounded subtask are compiled.
traceability:
...
5. Operational overlap must remain explicit until the LOTUSim HDDL profile defines a valid compilation for continuous tasks.
...
No normative HDDL method is emitted by this pilot. Classical ordering can be projected directly, but `spans` cannot be replaced by an arbitrary sequence without losing the requirement that following and protection remain active during transit.

The future HDDL profile must choose and validate one of the following strategies:

- temporal HDDL support, if the selected planner supports the required subset;
- lifecycle compilation into explicit start, maintain/check and stop operators;
- bounded discretization into transit segments with protection checks between segments.

The selected strategy must preserve failure propagation and must be tested on MC-026 before these methods can move from `pilot` to `draft`.
