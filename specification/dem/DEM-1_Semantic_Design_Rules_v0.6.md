...
## SDR-15 — State references shall resolve to one canonical tuple

Every state used by an enriched signature shall reference a normative State Model identifier through `state_ref`. Candidate symbols and deferred State Model candidates are not valid normative task references.

The bindings of a state reference shall match exactly the ordered argument roles declared by the State Model entry. Each bound parameter type shall be compatible with the corresponding State Model argument type. The same State Model identifier shall never be used with different tuple shapes. When several task variants share an operational concept, their arguments shall be normalized through a common type or split into distinct states.

State identifiers are stable and opaque. A symbol, category or implementation mapping may evolve without changing the identifier.
...
- Every task `state_ref` resolves to a normative State Model entry.
- Every state binding exactly matches the canonical State Model argument roles.
- Every bound parameter type is compatible with its State Model argument type.
...
# 8. Changes from v0.5 to v0.6

- Required stable `state_ref` identifiers for every enriched signature.
- Required task bindings to match one canonical State Model tuple signature.
- Established the first normative State Model as the only source of state symbols, categories, types, producers and consumers.

---

# 9. Changes from v0.4 to v0.5
