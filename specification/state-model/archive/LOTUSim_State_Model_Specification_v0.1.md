...
Version 0.1 is a deliberately bounded first increment. It covers the four semantic-pilot signatures and the seven core ISR signatures. It establishes the modeling and identifier conventions required before the remaining task signatures are enriched.
...
The v0.1 normative inventory contains:

- 38 states;
- 10 State Model-owned types;
...
No resource state is required by the current 11 signatures. The category remains part of the metamodel and will be populated by later sustainment and logistics increments.
...
| Category | Meaning | Examples in v0.1 |
|---|---|---|
...
| `resource` | Consumable or renewable quantity relevant to planning. | None in v0.1 |
...
State Model-owned types in v0.1 include information holders, observations, evidence, estimates, assessments and tracks. `SM-TY-010 navigation_objective` is an explicit union of `nmo:SpatialRegion` and `nmo:Route`; it gives `navigation_failed` one stable tuple shape for both Navigate signatures.

`information_holder` and `emission` remain ontology-mapping decisions. Their State Model identifiers allow work to continue without silently adding classes to the physical ontology. In v0.1, a platform or physical operational entity may realize `information_holder`; this compatibility is explicit in the YAML and does not classify every physical entity as a knowledge holder in the ontology.
...
Five mission-level concepts are deliberately not promoted in v0.1:
...
Version 0.1 is the baseline for enriching the remaining 68 signatures. New states shall be added only when a task or mission requirement cannot reuse an existing definition. Each family increment shall update producers, consumers and deferred candidates before its task semantics are considered complete.
